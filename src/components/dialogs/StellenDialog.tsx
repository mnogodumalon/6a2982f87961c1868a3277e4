...
import type { Stellen, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Stellen';
...
interface StellenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Stellen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Stellen['fields'], 'beschaeftigungsart' | 'status_stelle'> & {
    beschaeftigungsart?: LookupValue | string;
    status_stelle?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  beschaeftigungsart: LOOKUP_OPTIONS['stellen']?.['beschaeftigungsart'] ?? [],
  status_stelle: LOOKUP_OPTIONS['stellen']?.['status_stelle'] ?? [],
};
...
export function StellenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: StellenDialogProps) {
  const [fields, setFields] = useState<Partial<Stellen['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Stellen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'stellen');
      await onSubmit(clean as Stellen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "bezeichnung": string | null, // Stellenbezeichnung\n  "abteilung": string | null, // Abteilung\n  "standort": string | null, // Standort\n  "beschaeftigungsart": LookupValue | null, // Beschäftigungsart (select one key: "vollzeit" | "teilzeit" | "minijob" | "praktikum" | "werkstudent" | "freelance") mapping: vollzeit=Vollzeit, teilzeit=Teilzeit, minijob=Minijob, praktikum=Praktikum, werkstudent=Werkstudent, freelance=Freelance\n  "eintrittsdatum": string | null, // YYYY-MM-DD\n  "beschreibung": string | null, // Stellenbeschreibung\n  "status_stelle": LookupValue | null, // Status der Stelle (select one key: "offen" | "besetzt" | "pausiert" | "geschlossen") mapping: offen=Offen, besetzt=Besetzt, pausiert=Pausiert, geschlossen=Geschlossen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Stellen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Stellen bearbeiten' : 'Stellen hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'beschaeftigungsart': (
      <div key="beschaeftigungsart" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="beschaeftigungsart"><SelectValue placeholder="Wähle eine Beschäftigungsart" /></SelectTrigger>
          <SelectContent>
    ...
    'eintrittsdatum': (
      <div key="eintrittsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="eintrittsdatum"
          placeholder="Wann soll die Person starten?"
          mode="date"
    ...
    'status_stelle': (
      <div key="status_stelle" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"bezeichnung": "Stellenbezeichnung", "abteilung": "Abteilung", "standort": "Standort", "beschaeftigungsart": "Beschäftigungsart", "eintrittsdatum": "Gewünschtes Eintrittsdatum", "beschreibung": "Stellenbeschreibung", "status_stelle": "Status der Stelle"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.STELLEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
