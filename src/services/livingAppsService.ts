...
import type { Stellen, Bewerbungen, Bewerber, CreateStellen, CreateBewerbungen, CreateBewerber } from '@/types/app';
...
export class LivingAppsService {
  // --- STELLEN ---
  static async getStellen(): Promise<Stellen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.STELLEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Stellen[];
    return enrichLookupFields(records, 'stellen');
  }
  static async getStellenEntry(id: string): Promise<Stellen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.STELLEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Stellen;
    return enrichLookupFields([record], 'stellen')[0];
  }
  static async createStellenEntry(fields: CreateStellen) {
    return callApi('POST', `/apps/${APP_IDS.STELLEN}/records`, { fields: cleanFieldsForApi(fields as any, 'stellen') });
  }
  static async updateStellenEntry(id: string, fields: Partial<CreateStellen>) {
    return callApi('PATCH', `/apps/${APP_IDS.STELLEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'stellen') });
  }
  static async deleteStellenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.STELLEN}/records/${id}`);
  }

  // --- BEWERBUNGEN ---
  static async getBewerbungen(): Promise<Bewerbungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BEWERBUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Bewerbungen[];
    return enrichLookupFields(records, 'bewerbungen');
  }
  static async getBewerbungenEntry(id: string): Promise<Bewerbungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BEWERBUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Bewerbungen;
    return enrichLookupFields([record], 'bewerbungen')[0];
  }
  static async createBewerbungenEntry(fields: CreateBewerbungen) {
    return callApi('POST', `/apps/${APP_IDS.BEWERBUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'bewerbungen') });
  }
  static async updateBewerbungenEntry(id: string, fields: Partial<CreateBewerbungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.BEWERBUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'bewerbungen') });
  }
  static async deleteBewerbungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BEWERBUNGEN}/records/${id}`);
  }

  // --- BEWERBER ---
  static async getBewerber(): Promise<Bewerber[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BEWERBER}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Bewerber[];
    return enrichLookupFields(records, 'bewerber');
  }
  static async getBewerberEntry(id: string): Promise<Bewerber | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BEWERBER}/records/${id}`);
    const record = { record_id: data.id, ...data } as Bewerber;
    return enrichLookupFields([record], 'bewerber')[0];
  }
  static async createBewerberEntry(fields: CreateBewerber) {
    return callApi('POST', `/apps/${APP_IDS.BEWERBER}/records`, { fields: cleanFieldsForApi(fields as any, 'bewerber') });
  }
  static async updateBewerberEntry(id: string, fields: Partial<CreateBewerber>) {
    return callApi('PATCH', `/apps/${APP_IDS.BEWERBER}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'bewerber') });
  }
  static async deleteBewerberEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BEWERBER}/records/${id}`);
  }
