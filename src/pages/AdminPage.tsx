...
import type { Stellen, Bewerbungen, Bewerber } from '@/types/app';
...
import { BewerbungenViewDialog } from '@/components/dialogs/BewerbungenViewDialog';
import { BewerberDialog } from '@/components/dialogs/BewerberDialog';
import { BewerberViewDialog } from '@/components/dialogs/BewerberViewDialog';
...
const STELLEN_FIELDS = [
  { key: 'bezeichnung', label: 'Stellenbezeichnung', type: 'string/text' },
  { key: 'abteilung', label: 'Abteilung', type: 'string/text' },
  { key: 'standort', label: 'Standort', type: 'string/text' },
  { key: 'beschaeftigungsart', label: 'Beschäftigungsart', type: 'lookup/select', options: [{ key: 'vollzeit', label: 'Vollzeit' }, { key: 'teilzeit', label: 'Teilzeit' }, { key: 'minijob', label: 'Minijob' }, { key: 'praktikum', label: 'Praktikum' }, { key: 'werkstudent', label: 'Werkstudent' }, { key: 'freelance', label: 'Freelance' }] },
  { key: 'eintrittsdatum', label: 'Gewünschtes Eintrittsdatum', type: 'date/date' },
  { key: 'beschreibung', label: 'Stellenbeschreibung', type: 'string/textarea' },
  { key: 'status_stelle', label: 'Status der Stelle', type: 'lookup/select', options: [{ key: 'offen', label: 'Offen' }, { key: 'besetzt', label: 'Besetzt' }, { key: 'pausiert', label: 'Pausiert' }, { key: 'geschlossen', label: 'Geschlossen' }] },
];
const BEWERBUNGEN_FIELDS = [
  { key: 'bewerber', label: 'Bewerber/in', type: 'applookup/select', targetEntity: 'bewerber', targetAppId: 'BEWERBER', displayField: 'vorname' },
  { key: 'stelle', label: 'Stelle', type: 'applookup/select', targetEntity: 'stellen', targetAppId: 'STELLEN', displayField: 'bezeichnung' },
  { key: 'eingangsdatum', label: 'Eingangsdatum der Bewerbung', type: 'date/date' },
  { key: 'status_bewerbung', label: 'Aktueller Status', type: 'lookup/select', options: [{ key: 'vorauswahl', label: 'Vorauswahl' }, { key: 'telefoninterview', label: 'Telefoninterview' }, { key: 'persoenliches_gespraech', label: 'Persönliches Gespräch' }, { key: 'angebot', label: 'Angebot gemacht' }, { key: 'eingestellt', label: 'Eingestellt' }, { key: 'abgelehnt', label: 'Abgelehnt' }, { key: 'zurueckgezogen', label: 'Zurückgezogen' }, { key: 'neu', label: 'Neu eingegangen' }] },
  ...
  { key: 'naechster_schritt', label: 'Nächster Schritt', type: 'string/text' },
  { key: 'datum_naechster_schritt', label: 'Datum des nächsten Schritts', type: 'date/date' },
  { key: 'notizen_prozess', label: 'Notizen zum Bewerbungsprozess', type: 'string/textarea' },
];
const BEWERBER_FIELDS = [
  ...
  { key: 'wohnort', label: 'Wohnort', type: 'string/text' },
  { key: 'bewerbungskanal', label: 'Bewerbungskanal', type: 'lookup/select', options: [{ key: 'linkedin', label: 'LinkedIn' }, { key: 'xing', label: 'XING' }, { key: 'indeed', label: 'Indeed' }, { key: 'stepstone', label: 'StepStone' }, { key: 'empfehlung', label: 'Empfehlung' }, { key: 'initiativbewerbung', label: 'Initiativbewerbung' }, { key: 'sonstiges', label: 'Sonstiges' }, { key: 'eigene_website', label: 'Eigene Website' }] },
  { key: 'lebenslauf', label: 'Lebenslauf', type: 'file' },
  { key: 'anschreiben', label: 'Anschreiben', type: 'file' },
  { key: 'weitere_dokumente', label: 'Weitere Dokumente', type: 'file' },
  { key: 'notizen_bewerber', label: 'Notizen zum Bewerber', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'stellen', label: 'Stellen', pascal: 'Stellen' },
  { key: 'bewerbungen', label: 'Bewerbungen', pascal: 'Bewerbungen' },
  { key: 'bewerber', label: 'Bewerber', pascal: 'Bewerber' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('stellen');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'bewerbungen': return (data as any).bewerbungen as Bewerbungen[] ?? [];
      case 'bewerber': return (data as any).bewerber as Bewerber[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'bewerbungen':
        lists.bewerberList = (data as any).bewerber ?? [];
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'bewerbungen' && fieldKey === 'bewerber') {
      const match = (lists.bewerberList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'bewerbungen' && fieldKey === 'stelle') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'bewerbungen': return BEWERBUNGEN_FIELDS;
      case 'bewerber': return BEWERBER_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'bewerbungen': return {
        create: (fields: any) => LivingAppsService.createBewerbungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBewerbungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBewerbungenEntry(id),
      };
      case 'bewerber': return {
        create: (fields: any) => LivingAppsService.createBewerberEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBewerberEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBewerberEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'bewerbungen' || dialogState?.entity === 'bewerbungen') && (
        <BewerbungenDialog
          open={createEntity === 'bewerbungen' || dialogState?.entity === 'bewerbungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'bewerbungen' ? handleUpdate : (fields: any) => handleCreate('bewerbungen', fields)}
          defaultValues={dialogState?.entity === 'bewerbungen' ? dialogState.record?.fields : undefined}
          bewerberList={(data as any).bewerber ?? []}
      ...
      {(createEntity === 'bewerber' || dialogState?.entity === 'bewerber') && (
        <BewerberDialog
          open={createEntity === 'bewerber' || dialogState?.entity === 'bewerber'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'bewerber' ? handleUpdate : (fields: any) => handleCreate('bewerber', fields)}
          defaultValues={dialogState?.entity === 'bewerber' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'bewerbungen' && (
        <BewerbungenViewDialog
          open={viewState?.entity === 'bewerbungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'bewerbungen', record: r }); }}
          bewerberList={(data as any).bewerber ?? []}
          stellenList={(data as any).stellen ?? []}
        />
      )}
      {viewState?.entity === 'bewerber' && (
        <BewerberViewDialog
          open={viewState?.entity === 'bewerber'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'bewerber', record: r }); }}
        />
      )}
